...
def sampleDataSet(period, inputpath, outputpath, suffix):
    file_list = [f for f in listdir(inputpath) if isfile(join(inputpath, f)) and f.endswith(suffix)]
    for f in file_list:
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        message = "Usag: datasampling.py <period> <input_path> <output_path> <suffix>"
        sys.exit(message)
    ...
    suffix = sys.argv[4]
    sampleDataSet(period, inputpath, outputpath, suffix)
